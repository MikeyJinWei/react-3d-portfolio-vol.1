...
import lostRobotScene from '../assets/3d/lost-robot.glb';
...
const LostRobot = ({
  ...
  const lostRobotRef = useRef();
  const { gl, viewport } = useThree();
  const { nodes, materials, animations } = useGLTF(lostRobotScene);
  const { actions } = useAnimations(animations, lostRobotRef);
  ...
  const handlePointerMove = (e) => {
    ...
    if (isRotating) {
      ...
      lostRobotRef.current.rotation.y += delta * 0.01 * Math.PI;
  ...
  useFrame(() => {
    ...
    if (!isRotating) {
      ...
      lostRobotRef.current.rotation.y += rotationSpeed.current;
    } else {
      // When rotating, determine the current stage based on island's orientation
      const rotation = lostRobotRef.current.rotation.y;
